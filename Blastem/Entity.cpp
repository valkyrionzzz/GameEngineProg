	...
	if (entities != NULL && bumpIntoSolids && (velocity.x != 0 || velocity.y != 0))
	{
		...
		for (list<Entity*>::iterator e = entities->begin(); e != entities->end(); e++) {
			...
			if (tmpCollisionTime < collisionTime)
			{
				...
				normalx = tmpNormalx;
				normaly = tmpNormaly;
				//point to closest colliding entity so far
				collidingEntity = *e;
			}
		}
		//after the loop
		...
		if (collisionTime < 1.0f) {
			...
			if (collidingEntity->type == "coin") {
				collidingEntity->poof = true;																		//dwaddwadawdawdawdawdadawdawdawddawdawdadawda
			}
			...
			//changback	//velocity.x = dotprod * normaly;
			//velocity.y = dotprod * normalx;

			//float knockbackX(5);
			//float knockbackY(5);

			velocity.x *= remainingtime;
			velocity.y *= remainingtime;
			if (abs(normalx) > 0.0001f) {
				if (velocity.x == 0 && pos.x < collidingEntity->pos.x) {
					pos.x -= 1;
				}
				...
				if (velocity.x > 0) {
					velocity.x = ((collidingEntity->knockbackX)*-1);
					pos.x = pos.x - 5;
					collidingEntity->pos.x += 20;
				}
				else if (velocity.x < 0) {
					velocity.x = +collidingEntity->knockbackX;
					pos.x = pos.x + 5;
					collidingEntity->pos.x -= 20;
				}
			}
			...
			if (abs(normaly) > 0.0001f) {
				if (velocity.y >= 0) {
					velocity.y = ((collidingEntity->knockbackY)*-1);
					pos.y = pos.y - 5;
				}
				else if (velocity.y < 0) {
					velocity.y = +collidingEntity->knockbackY;
					pos.y = pos.y + 5;
				}
			}


			//if collidingEntity dmg > 0 AND my canTakeDMG == tru
			//take hp off me
			if (collidingEntity->dmg > 0 && canTakeDmg == true) {
				hp -= collidingEntity->dmg;
			}
